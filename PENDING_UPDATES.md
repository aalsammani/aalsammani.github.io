...
| 16 🟠 | **Calculus and Matrix Methods for Data Science** — requested as a featured open educational project, but no URL or description exists in the source files, on the live site, or via web search. | Not shown. | Send the URL (repository or site) and a one sentence description; add it to `_data/resources.json`. |
| 17 🟢 | **Research figures** — no publication figures exist in the repository; Featured Research uses schematic SVGs drawn for the site. | Schematics. | Provide figures (with rights) as PNG/SVG in `assets/img/` and point `_data/featured.json` at them. |
| 18 🟢 | **Code repositories** — no repository is linked from the site; GitHub's API was rate limited during the audit. | Resources section links to the GitHub profile only. | Add specific repositories (identifiability tutorial code, ICU model) to `_data/resources.json`. |
| 19 🟢 | **EdArXiv companion paper** — cited on Teaching without a link; no URL was supplied. | Text only. | Provide the EdArXiv URL/DOI. |
